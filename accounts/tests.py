...
class TestAccountsView(TestCase):
    ...
    def test_get_home_page(self):
        """when surfing website url testing
        if home page redirects to /products/ correctly"""
    ...
    def test_login_loads_correctly(self):
        """test login with redirect to products page
    ...
    def test_login_when_user_already_logged_in(self):
        """Tesing Login when user already logged in,
        with redirect to main page"""
        page = self.client.get('/', follow=True)
        self.user = User.objects.create_user(
            username='test', password='testpassword')
    ...
    def test_logout_correctly_logs_user_out(self):
        """testing logout with success message
    ...
    def test_registration(self):
        """test registration if user already logged in
...
class TestUserProfileModel(TestCase):
    ...
    def test_create_user_with_profile(self):
        ...
        self.assertTrue(
            hasattr(self.user.userprofile, 'street_address1'), True)
        self.assertTrue(
            hasattr(self.user.userprofile, 'street_address2'), True)
...
class TestProfilePage(TestCase):
    ...
    def test_profile_page(self):
        """
        testing profile page and if user model gets a profile
         as per extended model
        """
        self.user = User.objects.create_user(
            username='test', password='testpassword')
        self.client.login(username='test', password='testpassword')
